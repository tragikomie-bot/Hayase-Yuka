...
await page.locator('.addbtn').click();await page.locator('#amount').fill('800');await page.locator('#entry-currency').selectOption('JPY');await page.waitForFunction(()=>document.querySelector('#fx-result').textContent.includes('38.82'));await page.locator('#note').fill('日元购物 · 自动折算');await page.locator('#category').selectOption('购物');await page.locator('#entry-save').click();let db=await getDB();assert.equal(db.entries[2].converted,'38.82');assert.equal(db.entries[2].amount,'800.00');assert.equal(db.entries[2].rate,'0.048523');
await page.reload();assert.equal(await page.locator('.entry').count(),3);assert.match(await page.locator('.balance').textContent(),/7,935.58/);
